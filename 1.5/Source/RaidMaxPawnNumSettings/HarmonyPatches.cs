...
namespace CompressedRaid
{
    ...
    public static class Patch_MapGenerator_GenerateMap
    {
        ...
        public static void Postfix(Map __result)
        {
            ...
            {
                Pawn pawn = hostilePawns[i];
                if (CompressedRaidMod.AllowCompress(pawn) && gainStatValue > 0f && !disableFactors && PowerupUtility.EnableEnhancePawn(i, enhancePawnNumber))
                {
                    var hediff = PowerupUtility.RemoveAndSetPowerupHediff(pawn, order);
                    if (hediff != null)
                    {
                        bool success = PowerupUtility.TrySetStatModifierToHediff(hediff, gainStatValue);
                        if (success)
                        {
                            enhancedCount++;
                        }
            ...
            if (CompressedRaidMod.displayMessageValue && enhancedCount > 0)
            {
                Messages.Message($"[CR] {enhancedCount} preexisting enemies buffed after map generation.", MessageTypeDefOf.NeutralEvent, false);
            }
        }
    }
